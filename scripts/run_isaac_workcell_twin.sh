...
'; then
  echo "Isaac Sim environment does not match the workcell twin: $ISAAC_ENV" >&2
  exit 2
fi
...
if [[ ! -f "$ARENA_DIR/generated/isaac/6.0.1-watson-qc-10mm/tm5s_with_2fg7/tm5s_with_2fg7.usda" ]]; then
  echo "The Watson single-QC 10 mm baseline USD is missing; import the watson_qc_nominal profile first." >&2
  exit 4
fi
if [[ ! -f "$ARENA_DIR/config/workcell_scan.yaml" ]]; then
  echo "The workcell scan config is missing: $ARENA_DIR/config/workcell_scan.yaml" >&2
  exit 5
fi
...
cd "$ARENA_DIR"
exec "$ISAAC_ENV/bin/python" scripts/run_isaac_workcell_twin.py "$@"
