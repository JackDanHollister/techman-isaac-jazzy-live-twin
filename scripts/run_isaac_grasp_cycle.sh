...
CONFIG="$ARENA_DIR/config/isaac_grasp_cycle.yaml"
...
'; then
  echo "Isaac Sim environment does not match the grasp-cycle demo: $ISAAC_ENV" >&2
  exit 2
fi
...
if [[ ! -f "$CONFIG" ]]; then
  echo "Missing grasp-cycle config: $CONFIG" >&2
  exit 4
fi
if [[ ! -f "$USD" ]]; then
  echo "Missing articulated grasp-cycle USD: $USD" >&2
...
cd "$ARENA_DIR"
exec "$ISAAC_ENV/bin/python" scripts/run_isaac_grasp_cycle.py "$@"
